...
### 1. 런타임 커스텀 차트 주입 (`ChartInjector`) ★
- **무후킹 안전 주입:** `byref`/`Span` 트램펄린 크래시를 유발하는 파서(`_Gab`)나 로더(`_Ib`) 후킹 없이, 로드 완료된 런타임 인스턴스(`LogicalNotePlayer._Qe._ve`)를 직접 제어.
- **노트 치환 및 레인 조작 검증:** side(`_CA`)별 컨테이너(`_hA`)를 조작하여 132개 노트를 1개로 치환 및 가로 좌표 이동 성공.
- **렌더링 vs 판정 레인 분리 규명:** 렌더링 좌표(`_Ce`/`_de`)와 로더 계산 판정 레인(`_He`/`_ie`)을 완벽히 동기화하여 옮겨진 레인에서 정상 판정 확인.
