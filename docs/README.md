...
| **[02. 런타임 차트 주입 ★](02-런타임-차트-주입.md)** | **커스텀 차트의 핵심.** 노트 치환·레인 조작, 파서 출력 교체(`_VA` 네이티브 훅)와 노트 필드 의미 |
...
| **[06. 커스텀 채보 — BMS ★](06-커스텀-채보-작성.md)** | hwa 채보를 BMS 로 — 지금 되는 것(뼈대), 정해진 것, 다음에 고민할 것 |
...
| **커스텀 차트** | 차트 로더 `_VA` 네이티브 훅으로 파서 출력 교체 — 텍스트 채보가 **인게임 동작 확인** (2026-09-24) | **보통** |
